...
const Manager = () => {
        ...
        if (form.site.length >= 3 && form.username.length >= 3 && form.password.length >= 3) {
            ...
            if (saveButtonRef.current.innerHTML.includes("Save this edit")) {
                let res = await fetch("http://localhost:3000/", {
                    method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...form, button: "edit" })
                })
                getPasswords()
    ...
    const handleEdit = async (item) => {
        setForm({ site: item.site, username: item.username, password: item.password, id: item.id })
    ...
    const handleDelete = async (item) => {
        ...
        if (c) {
            ...
            setpasswordsArray(passwordsArray.filter(paitem => paitem.id !== item.id))
    ...
    return (
        <div className='flex flex-col justify-center items-center p-2 bg-white'>
                ...
                <div className=''>
                    ...
                    <div className='md:flex justify-between'>
                        ...
                        <div className='md:w-2/5 p-1'>
                            <input type="email" value={form.username} onChange={handleChange} name='username' placeholder='Enter Username' className='border-1 border-purple-500 rounded-sm w-full ' ref={usernameRef} required />
                        </div>

                        <div className='md:flex md:items-center relative md:w-2/5 p-1'>
                            {/* ------------password input--------------- */}
                            <input type="password" value={form.password} onChange={handleChange} name='password' placeholder='Enter Password' className='border-1 border-purple-500 rounded-sm w-full' ref={passwordRef} required />
                    ...
                    <div className='md:flex justify-center items-center p-1'>
                        ...
                        <div className='max-md:flex justify-center '>
                            <button onClick={savePassword} className='my-button flex justify-center items-center m-1 p-1 rounded-md border-1 border-purple-500 hover:cursor-pointer bg-purple-500 text-white' ref={saveButtonRef}>
                                <lord-icon
                                    src="https://cdn.lordicon.com/sbnjyzil.json"
                                    trigger="hover"
                                    stroke="bold"
                        ...
                        <button onClick={handleDummyPassword} className='flex justify-center items-center m-1 p-2 rounded-md border-1 border-purple-500 hover:cursor-pointer bg-purple-500 text-white'>
            ...
            <div className='pt-2'>
                ...
                />
                <div className='p-1'>
                    <hr className='opacity-0' /><hr className='opacity-0' /><hr className='opacity-0' />
                    <hr className='opacity-0' /><hr className='opacity-0' /><hr className='opacity-0' />
                    <hr className='opacity-0' /><hr className='opacity-0' /><hr className='opacity-0' />
                    <hr className='opacity-0' /><hr className='opacity-0' /><hr className='opacity-0' />
                                    ...
                                    <tr key={index}>
                                        <td className='border-1'>
                                            <div className='flex items-center justify-between'>
                                                <span className='w-11/12 overflow-x-auto'><a href={item.site} target="_blank" rel="noopener noreferrer" className="">{item.site}</a></span>
                                                <div className='hover:cursor-pointer' onClick={() => { handleCopy(item.site) }}><img src="icons/copy.svg" alt="copy.svg" /></div>
                                            </div>
                                        </td>
                                        <td className='border-1'>
                                            <div className='flex items-center justify-between'>
                                                <div className='w-[85%] max-sm:overflow-x-scroll'>{item.username}</div>
